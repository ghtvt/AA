...
var rule = {
   ...
   一级: async function () {
	...
	二级: async function () {
        ...
        VOD.vod_content = pdfh(html, '.cor3&&Text');
	...
	lazy: async function() {
      ...
      if (/m3u8|mp4/.test(url)) {
          return {jx: 0, url: url, parse: 0}
          
      } else {
          let Config = {};
          let jscode =await request(HOST + '/static/js/playerconfig.js');
          ...
          log("jx>>>>>" + jx);
          try {
              let jxhtml =await request(jx + url, {
                    headers: {
              ...
              console.log('解密的url:' + playurl);
               return {jx: 0, url: playurl, parse: 0}
          } catch (error) {
